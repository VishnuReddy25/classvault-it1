...
export default function MemoryCard({ memory: m, vaultId, index = 0 }) {
  const { react, addComment, deleteMemory, activeVault } = useApp();
  ...
  const canDelete = !locked && (isAdmin || m.uploader === userName);

  const handleReact = (key) => { if (locked) return; react(vaultId, m.id, key); };

  const handleComment = () => {
    if (!comment.trim() || locked) return;
    addComment(vaultId, m.id, { id: Date.now(), user: userName, text: comment.trim() });
    setComment('');
  };
  ...
  return (
    ...
    >
      {/* Delete button — top-right, only for uploader/admin */}
      {canDelete && (
        <button
          onClick={handleDelete}
          onMouseLeave={() => setConfirmDel(false)}
          title={confirmDel ? 'Click again to confirm' : 'Delete this memory'}
          style={{
            position:'absolute', top:10, right:10, zIndex:10,
            padding:'4px 12px', borderRadius:100, fontSize:11, fontWeight:600,
            background: confirmDel ? '#ef4444' : 'rgba(239,68,68,0.85)',
